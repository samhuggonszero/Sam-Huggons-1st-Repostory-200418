...
on le.event_name=x.event_name
and le.client_id=x.client_id 
	where le.ts >= date_trunc('month',getdate()-31) and le.ts < DATEADD(month,-1,date_trunc('day',getdate())) 
	group by 1,2
	order by 1,2
  ) c
  group by 1
...
max(case when le.client_id in (6) then 1 else 0 end) as desktop,
max(case when le.client_id in (7,808) then 1 else 0 end) as mobile

from mendeley.active_users_by_client_per_day le
where index_ts>='2017-09-01' and index_ts<'2017-10-01'
group by 1) a
group by 1,2,3
